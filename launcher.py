...
cs.store(name=DEFAULT_CONFIG_NAME, node=config)  # put `config` in there
...
def main(cfg: Config) -> None:
    ...
    if cfg.slurm.enabled:
        ...
        if cfg.slurm.job_launch_delay_hours is not None:
            executor.update_parameters(additional_parameters={"begin": f"now+{cfg.slurm.job_launch_delay_hours}hours"})

    # CL overrides
    overrides: list[str] = OmegaConf.to_object(hydra_cfg.overrides.task)  # pyright: ignore[reportAssignmentType]
...
class Task:
    ...
    def __call__(self):
        ...
        for (
            ...
            if cfg_item_value is True or cfg_item_value in ["True", "true"]:
                ...
                pass
            elif isinstance(cfg_item_value, int | str | float):
                accelerate_cfg += f"--{cfg_item_name} {cfg_item_value} "
            else:
                self.logger.warning(f"Not passing arg '{cfg_item_name}' with value {cfg_item_value} to accelerate CLI")
    ...
    def checkpoint(self):
        ...
        # (ie resume_from_checkpoint=True), then change that argument as an override
        # to resume from the last checkpoint!
        if (
            self.cfg.checkpointing.resume_from_checkpoint is not True
            and "checkpointing.resume_from_checkpoint=True" not in self.overrides
        ):
            self.overrides.append("checkpointing.resume_from_checkpoint=True")
...
GITIGNORE_CONTENT = f"""# Ignore everything by default
*
# Keep the copied code snapshot visible to Git
!{CODE_FOLDER_NAME}/
!{CODE_FOLDER_NAME}/**
# Keep the copied config snapshot visible to Git
!{CONFIG_FOLDER_NAME}/
!{CONFIG_FOLDER_NAME}/**
"""

# do not git diff the user *inference* config when launching train jobs here (for now: TODO use it!)
GITDIFF_EXCLUDE = {f"{CONFIG_FOLDER_NAME}/my_inference_conf.py", "*.pyc"}


def _get_specific_diffs(
    repo: Repo,
    prev_commit: str,
    new_commit: str,
    path: str | None = None,
    ...
    Args:
        ...
        exclude_paths: Paths to exclude from diff output

    Returns:
        Formatted diff as string
    """
    args = ["--unified=0", "--color", prev_commit, new_commit]
    if path is not None or exclude_paths is not None:
        args.append("--")
        if path is not None:
            args.append(path)
        if exclude_paths is not None:
            args.extend([f":(exclude){excluded_path}" for excluded_path in exclude_paths])

    return repo.git.diff(*args)


def _get_code_changes_summary(
    repo: Repo, prev_commit: str, new_commit: str, exclude_paths: list[str] | None = None
) -> dict[str, int]:
    """
    ...
    if exclude_paths is not None:
        changed_files = [f for f in changed_files if not any(f.startswith(path) for path in exclude_paths)]
...
def prepare_and_confirm_launch(cfg: Config, hydra_cfg: HydraConf, logger: Logger) -> tuple[Path, Path, Path]:
    ...
    with open(gitignore_path.as_posix(), "w") as f:
    ...
    match diff_mode:
        case "full":
            ...
            full_diff = _get_specific_diffs(repo, prev_commit.hexsha, new_commit.hexsha, exclude_paths=GITDIFF_EXCLUDE)
            logger.info(f"Git diff with previous commit: {prev_commit.message}\n{full_diff}")
        case "config_only":
            # Show only config diff + code summary
            logger.debug("Diff mode set to 'config_only': showing only config diff and code changes summary")
            config_diff = _get_specific_diffs(
                repo,
                prev_commit.hexsha,
                new_commit.hexsha,
                CONFIG_FOLDER_NAME,
    ...
    if cfg.debug:
        ...
        try:
            ...
            if diff_mode != "full":
                confirmation = input("\n   => Show code diff additionally? (y/[n]): ")
