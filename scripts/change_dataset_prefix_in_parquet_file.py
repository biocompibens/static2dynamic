...
def main(args: argparse.Namespace):
    ...
    for ds_files in args.dataset_files_list:
        ...
        # existence check
        if args.no_check:
            print("\nSkipping existence check as per --no_check flag.")
        else:
            print(f"\nChecking that {len(new_file_paths)} files exist...", flush=True)
            first_missing = next((p for p in tqdm(new_file_paths) if not Path(p).exists()), None)
            if first_missing is not None:
                print("")
                if args.debug:
                    print(f"\nERROR: file does not exist: {first_missing}")
                else:
                    raise FileNotFoundError(f"File does not exist: {first_missing}")
            else:
                print("All good!")
