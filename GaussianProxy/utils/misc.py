...
def create_repo_structure(
    ...
    |   |   |   checkpoints
    |   |   |   |   step_<x>
    |   |   |   |   step_<x+checkpointing_every_n_steps>
    |   |   |   |   .tmp_inference_save
    |   |   |   {best,last}_model
    |   |   |   {train,test}_samples.{json,parquet}
    |   |   |   saved_artifacts
    |   |   |   .hydra
    |   |   |   .git
    |   |   |   .gitignore
    ...
    # Create a folder to save the best / last model during training
    best_model_save_folder = Path(this_run_folder, "best_model")
    if accelerator.is_main_process:
        best_model_save_folder.mkdir(exist_ok=True)
    last_model_save_folder = Path(this_run_folder, "last_model")
    if accelerator.is_main_process:
        last_model_save_folder.mkdir(exist_ok=True)
    ...
    return best_model_save_folder, last_model_save_folder, saved_artifacts_folder, chckpt_save_path
...
def modify_args_for_debug(
    ...
    for strat in cfg.evaluation.strategies:
        if hasattr(strat, "nb_diffusion_timesteps"):
        ...
        if hasattr(strat, "selected_times") and strat.selected_times is not None and len(strat.selected_times) > 2:  # pyright: ignore[reportAttributeAccessIssue]
...
ACCEPTED_NORMALIZATIONS = Literal["image min-max", "image 5perc-95perc", "video min-max", "-1_1 raw", "-1_1 clipped"]

# Define per-channel colors: [R, G, B], in [0, 1]
LUT_4_CH_TO_3_CH = torch.tensor(
    [
        [1.0, 0.0, 0.0],  # Red
        [0.0, 1.0, 0.0],  # Green
        [0.0, 0.0, 1.0],  # Blue
...
def save_eval_artifacts_log_to_wandb(
    ...
    logging_normalization: list[ACCEPTED_NORMALIZATIONS],
    max_nb_to_save_and_log: int = 16,
    captions: None | list[None] | list[str] = None,
    split_name: str | None = None,
    use_lut_for_4ch: bool = False,
):
    """
    ...
    if sel_to_save.shape[tensors_to_save.ndim - 3] == 1:
        ...
        if use_lut_for_4ch:
            ...
            logger.debug("Composing RGB images from 4-channel images using predefined LUT")
            lut = LUT_4_CH_TO_3_CH.to(sel_to_save.device, sel_to_save.dtype)
            sel_to_save = torch.einsum("...chw,cm->...mhw", sel_to_save, lut)
        else:
            # split each 4 channels images into a pair of 2 images:
            # one with the 3 first channels and another with the last one repeated 3 times
            # (for compatibility with wandb logging)
            logger.debug("Splitting 4-channel images into RGB pairs")
            rgb = sel_to_save[..., :3, :, :]  # (B, (T,) 3, H, W)
            last_channel = sel_to_save[..., (3,), :, :]  # (B, (T,) 1, H, W)
            if sel_to_save.ndim == 5:
                repeated_last_channel = last_channel.repeat(1, 1, 3, 1, 1)  # (B, T, 3, H, W)
            else:
                repeated_last_channel = last_channel.repeat(1, 3, 1, 1)  # (B, 3, H, W)
            # now cat these tensors in an interleaved way
            _interleaved_cat = []
            for i in range(len(rgb)):
                _interleaved_cat.extend([rgb[i], repeated_last_channel[i]])
            sel_to_save = torch.stack(_interleaved_cat)  # (2*B, (T,) 3, H, W)
            sel_captions = [c for c in sel_captions for _ in range(2)]
    else:
    ...
    assert sel_to_save.shape[tensors_to_save.ndim - 3] == 3, (
        f"Expected 3 channels for ready-to-be-logged images/videos, got shape {sel_to_save.shape}"
    )
...
@torch.inference_mode()
def normalize_elements_for_logging(
    elems: Tensor,
    logging_normalizations: list[ACCEPTED_NORMALIZATIONS],
) -> dict[str, ndarray]:
    """
    ...
    for norm in logging_normalizations:
...
def warn_about_dtype_conv(
    ...
    if model.dtype != target_dtype:
        logger.warning(f"Original model dtype: {model.dtype}; converting to target dtype: {target_dtype}")
    if model.dtype == torch.bfloat16 and target_dtype not in (
        torch.bfloat16,
        torch.float32,
    ):
        logger.error(f"model is bf16 but target_dtype is {target_dtype}: conversion might be weird/invalid")


def shorten(s: str, max_total: int = 60) -> str:
    """
    Shorten long names à la zsh path shrinking.
    """
    tokens = s.split("_")

    def abbrev(tok: str) -> str:
        if tok.isnumeric():
            return tok
        if "=" in tok:
            k, v = tok.split("=", 1)
            if k:
                uppers = "".join(c for c in k if c.isupper())
                k_short = uppers if len(uppers) >= 2 else k[:3].lower()
            else:
                k_short = k
            return f"{k_short}={v}"
        if any(ch.isdigit() for ch in tok):
            ...
            if m:
                ...
                initials = "".join(w[0] for w in re.findall(r"[A-Za-z]+", rest))
                return f"{digits}{initials or ''}"
            return tok[:4]
        uppers = "".join(c for c in tok if c.isupper())
        if len(uppers) >= 2:
            return uppers
        if len(tok) <= 4:
            return tok
    ...
    head = [abbrev(t) for t in tokens]
    out = "_".join(head)
    if len(out) <= max_total:
        return out

    # Further compact the middle if still too long
    if len(tokens) > 4:
        left = tokens[:2]
        mid = tokens[2:-2]
        right = tokens[-2:]
        compact_mid = [(t[0] if t else "") for t in mid]
        out2 = "_".join([*(abbrev(t) for t in left), *compact_mid, *(abbrev(t) for t in right)])
        if len(out2) < len(out):
            return out2
    return out
