...
from torchvision.transforms.transforms import Compose as Compose_v1
from torchvision.transforms.transforms import Normalize as Normalize_v1
from torchvision.transforms.transforms import RandomCrop as RandomCrop_v1
...
from torchvision.transforms.v2 import RandomCrop as RandomCrop_v2
...
class TimeDiffusion:
    ...
    *not* the "diffusion process" discretization time(step).
    ...
    best_model_save_folder: Path = field(init=False)
    last_model_save_folder: Path = field(init=False)
    ...
    def fit(
        ...
        best_model_save_folder: Path,
        last_model_save_folder: Path,
        ...
        for time_or_times, batch in self._yield_data_batches(
            ...
            if self.global_optimization_step % self.cfg.checkpointing.checkpoint_every_n_steps == 0 and (
                ...
                self._checkpoint()
                # also perform a full isolated save in "last_model" in case we never have a "best_model" for some reason
                # (eg: if we don't perform any metrics computation eval!)
                self.logger.info("Saving last model to date")
                self._save_pipeline(self.last_model_save_folder)

            # evaluate models every n optimisation steps
            performed_eval_this_step = False
            if (
            ...
            ### And finally², if eval was performed at this step, checkpoint again
            # to avoid reruning the same (lengthy) evaluation when resuming the last checkpoint
            if performed_eval_this_step:
                self._checkpoint()
    ...
    def _fit_init(
        ...
        if not self._nb_empirical_dists > 1 and fully_ordered_train_ds is None:
            raise ValueError("Expecting at least 2 empirical distributions to train the discrete model.")
        ...
        self.last_model_save_folder = last_model_save_folder
    ...
    def _evaluate(
        ...
        for eval_strat in self.cfg.evaluation.strategies:  # TODO: match on type when config is updated
            ...
            if eval_strat.name.startswith("SimpleGeneration"):
                self._simple_gen(
                    pbar_manager,
            ...
            elif eval_strat.name.startswith("ForwardNoising"):
            ...
            elif eval_strat.name.startswith("InvertedRegeneration"):
                if self.cfg.dataset.fully_ordered:
                    ...
                    self.logger.debug(
                        f"Using {self.timesteps_floats_to_names[0]} timestep name to filter on unique true labels: {self.fully_ordered_train_ds.df['true_label'].unique()}"
                    )
                    ...
                    sampled_train_files_time0 = all_train_files_time0.sample(eval_strat.nb_generated_samples)  # pyright: ignore[reportAttributeAccessIssue]
                    ...
                    sampled_test_files_time0 = all_test_files_time0.sample(eval_strat.nb_generated_samples)  # pyright: ignore[reportAttributeAccessIssue]
                    ...
                    train_batch_time0: Tensor = next(iter(train_dl_time0))[: eval_strat.nb_generated_samples]  # pyright: ignore[reportAttributeAccessIssue]
                    while train_batch_time0.shape[0] != eval_strat.nb_generated_samples:  # pyright: ignore[reportAttributeAccessIssue]
                        train_batch_time0 = torch.cat([train_batch_time0, next(iter(train_dl_time0))])
                        train_batch_time0 = train_batch_time0[: eval_strat.nb_generated_samples]  # pyright: ignore[reportAttributeAccessIssue]
                ...
                assert (
                    expected_shape := (
                        eval_strat.nb_generated_samples,  # pyright: ignore[reportAttributeAccessIssue]
                        *self.data_shape,
                    )
            ...
            elif eval_strat.name.startswith("IterativeInvertedRegeneration"):
            ...
            elif eval_strat.name.startswith("MetricsComputation"):
            ...
            elif eval_strat.name.startswith("SimilarityWithTrainData"):
    ...
    def _simple_gen(
        ...
        save_eval_artifacts_log_to_wandb(
            ...
            eval_strat.name,
            "simple_generations",
            ["-1_1 raw", "image min-max", "image 5perc-95perc"],
    ...
    def _similarity_with_train_data(
        ...
        data_transforms: Compose_v1 | Compose_v2,
    ):
        """
        ...
        # TODO: precompute true images once
        """
        # Checks
        ...
        ##### 0. Preparations
        # Set precision flags
        prev_fp32_precision_setting = torch.backends.fp32_precision  # pyright: ignore[reportAttributeAccessIssue]
        ...
        kept_transforms_for_gen = Compose_v2(
            [t for t in kept_transforms.transforms if not isinstance(t, RandomCrop_v1 | RandomCrop_v2)]
        )
        ...
        for gen_img_idx, gen_img in enumerate(gen_dl):
            ...
            gen_img = gen_img.to(self.accelerator.device, non_blocking=True)  # (B_gen, C, H, W)
            ...
            true_dl = DataLoader(
                ...
                num_workers=4,  # not too much because we are already distributed here!
                pin_memory=True,
                prefetch_factor=2,
            ...
            for true_img_batch_output in true_dl:
                ...
                true_img_batch = true_img_batch.to(self.accelerator.device, non_blocking=True)  # (B_true, C, H, W)
        ...
        torch.backends.fp32_precision = prev_fp32_precision_setting  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def _metrics_computation(
        ...
        if isinstance(eval_strat.nb_samples_to_gen_per_time, str) and "aug" in eval_strat.nb_samples_to_gen_per_time:
            ...
            if self.accelerator.is_main_process:
                ...
                ] + ([metrics_computation_folder / "all_times"] if eval_strat.also_compute_metrics_on_all_times else [])
                hard_augment_dataset_all_square_symmetries(
                    subdirs_to_augment,
                    self.logger,
                    extension,
        ...
        # TODO: differentiate between using seen data only and all the available dataset with the "all_times" task
        ...
        if new_best_metric_to_date is not False:
            ...
            self._save_pipeline(self.best_model_save_folder)
    ...
    def _find_this_proc_this_time_batches_for_metrics_comp(
        ...
        if isinstance(eval_strat.nb_samples_to_gen_per_time, int):
            ...
            tot_nb_samples = nb_all_aug_samples // (base_aug_factor)
            # all samples, *then* base_aug_factor⨉ augment them
            ...
            tot_nb_samples = nb_all_aug_samples // (base_aug_factor * 2)
            # half the number of samples, *then* base_aug_factor⨉ augment them
            self.logger.debug(f"Will augment samples {base_aug_factor}⨉ after generation")
        else:
            raise ValueError(
                f"Expected 'nb_samples_to_gen_per_time' to be an int, 'adapt', 'adapt half', 'adapt aug', or 'adapt half aug', got {eval_strat.nb_samples_to_gen_per_time}"
            )
        self.logger.debug(
    ...
    def _get_true_datasets_for_metrics_computation(
        ...
        test_transforms: Compose_v1 | Compose_v2 = test_dataloaders[0].dataset.transforms  # pyright: ignore[reportAttributeAccessIssue]
        ...
        metrics_compute_transforms = Compose_v2(
            [
                ...
                ToDtype(TORCH_DTYPES[eval_strat.dtype], scale=True),  # no-op if already f32
        ...
        if (
            ...
            # get hard aug ds path
            if (
                not OmegaConf.is_missing(self.cfg.dataset, "hard_aug_path")
                and self.cfg.dataset.hard_aug_path is not None
            ):
                hard_aug_ds_path = self.cfg.dataset.hard_aug_path
            else:
                hard_aug_ds_path = base_dataset_path.with_name(base_dataset_path.name + "_hard_augmented")
            # now samples per time
            for time_name in eval_time_names:
                hard_aug_ds_this_time_path = hard_aug_ds_path / time_name
        ...
        # Then if generating half the number of samples, take half of the available true data to compare
        ...
        # add the 'all_times' dataset if needed
        if eval_strat.also_compute_metrics_on_all_times:
            all_samples_search_path = hard_aug_ds_path if hard_aug_ds_path is not None else base_dataset_path
            all_files_all_times = list(all_samples_search_path.rglob(f"*{common_suffix}", recurse_symlinks=True))
            if self.debug:
    ...
    def _save_pipeline(self, save_path: Path, called_on_main_process_only: bool = False):
        """
        ...
        Can be called by all processes (only main will actually save), or by main only
        (in which case no dist barrier is inserted inside here).
        """
        # net
        self.accelerator.unwrap_model(self.net).save_pretrained(
            save_path / "net",
        ...
        # video time encoder
        self.accelerator.unwrap_model(self.video_time_encoding).save_pretrained(
            save_path / "video_time_encoder",
            is_main_process=self.accelerator.is_main_process,
        )
        if self.accelerator.is_main_process:
            # dynamic config
            self.dynamic.save_pretrained(save_path / "dynamic")
            # resuming args
            ...
            resuming_args.to_json(save_path / ResumingArgs.json_state_filename)

        if not called_on_main_process_only:
            self.accelerator.wait_for_everyone()

        self.logger.info(f"Saved net, video time encoder, dynamic config, and resuming args to {save_path}")
...
def load_resuming_args(
    ...
    `cfg.checkpointing.resume_from_checkpoint` can be:
    - `int`: resume from a specific step
    - `True`: resume from the most recent checkpoint
    - `"last_model_save"`: resume from the last *saved model* (`!=` checkpoint!)
    """
    ...
    elif resume_arg == "last_model_save":
        resuming_path = last_model_save_folder
    else:
        raise ValueError(
            f"Expected 'checkpointing.resume_from_checkpoint' to be an int, True or 'last_model_save' at this point, got {resume_arg}"
        )
